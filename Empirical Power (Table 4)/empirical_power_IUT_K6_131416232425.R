rm(list = ls())
true.mean <- c(54.5,55.2,62.1,61.8,63.3,64.9)                        # Sample mean
K<-length(true.mean)                                                # No. of comparison groups 
psd<- 5.90                                                           # Pooled standard deviation

n_m<- c(11,11,7,7,6,6)  # max-min design
n_b= c(8,8,8,9,9,9)   # balance design
...
for (i in 1:m) { 
  ...
  t_stat_m[i]<- min(c(abs(Z_13_m),abs(Z_14_m),abs(Z_16_m),abs(Z_23_m),abs(Z_24_m),abs(Z_25_m)))                          #test statistics under the null for max-min design
  ...
  t_stat_b[i]<- min(c(abs(Z_13_b),abs(Z_14_b),abs(Z_16_b),abs(Z_23_b),abs(Z_24_b),abs(Z_25_b)))                          #test statistics under the null for balanced design
...
for (i in 1:m) { 
  ...
  t_stat_m.A<- min(c(abs(Z_13_m.A),abs(Z_14_m.A),abs(Z_16_m.A),abs(Z_23_m.A),abs(Z_24_m.A),abs(Z_25_m.A)))              #test statistics under the altenative for max-min design
  ...
  t_stat_b.A<- min(c(abs(Z_13_b.A),abs(Z_14_b.A),abs(Z_16_b.A),abs(Z_23_b.A),abs(Z_24_b.A),abs(Z_25_b.A)))                    #test statistics under the altenative for balanced design
