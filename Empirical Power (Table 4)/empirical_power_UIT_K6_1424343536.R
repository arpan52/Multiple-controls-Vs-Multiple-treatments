rm(list = ls())
true.mean <- c(52.3,53.8,53.8,56.6,54.8,55.2)                          # Sample mean
K<-length(true.mean)                                                   # No. of comparison groups  
psd<- 5.79                                                             # Pooled standard deviation

n_m<- c(8,9,12,12,9,8)  # max-min design
n_b= c(10,10,10,9,9,9)   # balance design
...
for (i in 1:m) {  
  ...
  t_stat_m[i]<- max(c(abs(Z_14_m),abs(Z_24_m),abs(Z_34_m),abs(Z_35_m),abs(Z_36_m)))                          #test statistics under the null for max-min design
  ...
  t_stat_b[i]<- max(c(abs(Z_14_b),abs(Z_24_b),abs(Z_34_b),abs(Z_35_b),abs(Z_35_b)))                          #test statistics under the null for balanced design
...
for (i in 1:m) { 
  ...
  t_stat_m.A<- max(c(abs(Z_14_m.A),abs(Z_24_m.A),abs(Z_34_m.A),abs(Z_35_m.A),abs(Z_36_m.A)))              #test statistics under the altenative for max-min design
  ...
  t_stat_b.A<- max(c(abs(Z_14_b.A),abs(Z_24_b.A),abs(Z_34_b.A),abs(Z_35_b.A),abs(Z_36_b.A)))                    #test statistics under the altenative for balanced design
